...
/**
 * Empty NEXT_PUBLIC_API_BASE => fixture mode (routes under /api/fixture).
 * Set it to B's FastAPI origin to switch the whole app onto real services:
 *   NEXT_PUBLIC_API_BASE=http://127.0.0.1:8000
 */
const BASE = process.env.NEXT_PUBLIC_API_BASE ?? "";

export const isFixtureMode = BASE === "";
...
function endpoint(name: "search" | "select" | "publish"): string {
  return BASE ? `${BASE}/api/${name}` : `/api/fixture/${name}`;
}
