...
import { ApiError, isFixtureMode, publish, search, select } from "@/lib/api";
import {
  STYLE_LABEL,
  type Card,
...
export default function Home() {
  ...
  async function onSearch(event: React.FormEvent) {
    ...
    const previous = result?.cards.map((c) => c.id) ?? null;
  ...
  function rankDelta(cardId: string, index: number): number | null {
    if (!previousOrder) return null;
    const before = previousOrder.indexOf(cardId);
